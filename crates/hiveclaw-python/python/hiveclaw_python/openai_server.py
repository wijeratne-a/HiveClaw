...
from .steering import ActiveSteeringWrapper, CaptureWrapper, check_latent_dim, load_sae
...
def _default_sae_path() -> Path:
    try:
        ...
        return find_repo_root() / "models" / "hiveclaw_sae_v1.safetensors"
    except RuntimeError:
        return Path("hiveclaw_sae_v1.safetensors")
...
def _sae_path() -> Path:
    return Path(
        os.environ.get("HIVECLAW_SAE_PATH", str(_default_sae_path()))
    )
MAX_CONCURRENT = 1
...
def _sync_startup() -> ServerContext:
    ...
    d_latent = int(slab_client.get_latent_dim())
    enc_latent, enc_hidden = int(W_enc.shape[0]), int(W_enc.shape[1])
    if enc_hidden != hidden_size:
        print(
            f"[ERROR] SAE encoder.weight second dim {enc_hidden} != model hidden_size {hidden_size}.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    if enc_latent != d_latent:
        print(
            f"[ERROR] SAE encoder latent dim {enc_latent} != slab get_latent_dim()={d_latent}.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
async def lifespan(app: FastAPI):
    ...
    if continuous:
        ...
        probed_max = await loop.run_in_executor(
            None, probe_max_batch, _model_id(), env_max_batch, probe_ctx
        )
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="HuggingFace / mlx-lm model id (default: env HIVECLAW_MODEL_ID or Llama-3.2-1B).",
    )
    p.add_argument(
        "--sae-path",
        type=str,
        default=None,
        help="Path to SAE safetensors (default: env HIVECLAW_SAE_PATH or models/hiveclaw_sae_v1.safetensors).",
    )
    p.add_argument(
        "--no-stigmergy",
        action="store_true",
    ...
    args = p.parse_args()
    if args.model_id:
        os.environ["HIVECLAW_MODEL_ID"] = args.model_id
    if args.sae_path:
        os.environ["HIVECLAW_SAE_PATH"] = args.sae_path
