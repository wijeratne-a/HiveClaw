...
def check_latent_dim(slab_client) -> None:
    """Slab v6: latent width comes from IOSurface header; only sanity-check."""
    d = int(slab_client.get_latent_dim())
    if d < 1:
        print(f"[ERROR] invalid get_latent_dim()={d}", file=sys.stderr)
        sys.exit(1)
...
def load_sae(sae_path: Path) -> dict[str, mx.array]:
    ...
    if W.ndim != 2:
        print(
            f"[ERROR] encoder.weight must be rank-2 [latent, hidden], got shape {W.shape}",
            file=sys.stderr,
        )
        sys.exit(1)
    return out
...
def steer_hidden(
    ...
    scent_latent = slab_client.read_slot_v5(int(slot_index), depends=h_step)
    scent_f = scent_latent.astype(mx.float32)
