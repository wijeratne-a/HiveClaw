...
- **Training pipeline:** `scripts/harvester.py` → `models/latent_traces_*.npz` → `scripts/train_sae.py` → `models/hiveclaw_sae_v1.safetensors` (ignored by git). Verify with `tests/test_sae_tied_weights.py`.
